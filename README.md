...
* The service is `Type=oneshot` with `Restart=on-failure` (backoff if
  enabled), so a transient failure is retried automatically.
* The service runs with strict systemd sandboxing (`ProtectSystem`,
  `ProtectHome`, `PrivateTmp`, read-only root, and hardened kernel
  protections) at reduced scheduling priority.
* The audit log is rotated automatically by logrotate if present.
